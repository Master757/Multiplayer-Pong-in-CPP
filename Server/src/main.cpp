    ...
    while (true) {
        ...
        for (auto& pair : roomManager.active_rooms) {
            ...
            if (room.isGame) {
                ...
                GameState currentState = room.session.state; 

                // BROADCAST: Send the truth to both players
                server.send_data(room.player1Socket, &currentState, sizeof(GameState));
                server.send_data(room.player2Socket, &currentState, sizeof(GameState));
            }
        }

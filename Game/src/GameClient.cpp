...
bool GameClient::reqCreateRoom(){ // Fixed: Capital G in GameClient
    ...
    clientHandshake gameClientPacket;
    memset(&gameClientPacket, 0, sizeof(gameClientPacket)); //memset FIRST
    gameClientPacket.action = 1; // Then set the action
...
bool const GameClient::reqJoinRoom(const std::string& code){
    ...
    clientHandshake gameClientPacket;
    memset(&gameClientPacket, 0, sizeof(gameClientPacket));

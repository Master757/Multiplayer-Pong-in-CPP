...
void RoomManager::handleDisconnect(int clientSocket) {
    /*
     * When a player disconnects, we need to:
     * 1. Find which room they were in
     * 2. Set their socket to -1 (so the server stops sending to them)
     * 3. Stop the game (can't play pong alone!)
     * 4. If BOTH players are gone, delete the room entirely
     *
     * We use an iterator loop (not range-for) because we might erase
     * entries while iterating. Erasing during a range-for = crash.
     */
    auto it = active_rooms.begin();
    while (it != active_rooms.end()) {
        Room& room = it->second;
        bool wasInRoom = false;

        if (room.player1Socket == clientSocket) {
            room.player1Socket = -1;  // Mark as disconnected
            wasInRoom = true;
            std::cout << "Player 1 disconnected from room " << it->first << "\n";
        }
        if (room.player2Socket == clientSocket) {
            room.player2Socket = -1;  // Mark as disconnected
            wasInRoom = true;
            std::cout << "Player 2 disconnected from room " << it->first << "\n";
        }

        if (wasInRoom) {
            room.isGame = false;  // Stop physics — can't play with 1 player

            // If both players are gone, delete the room
            if (room.player1Socket == -1 && room.player2Socket == -1) {
                std::cout << "Room " << it->first << " is empty, deleting.\n";
                it = active_rooms.erase(it);  // erase() returns the NEXT iterator
                continue;  // Skip the it++ below since erase already advanced
            }
        }
